...
function ClaimList({ title, claims }: { title: string; claims: { text: string; sourceQuerySequences: number[] }[] }) {
  return (
    <section className="border-t border-[var(--border-default)] py-6">
      ...
      {claims.length ? <div className="space-y-3">{claims.map((claim, index) => <div key={`${claim.text}-${index}`} className="flex gap-3"><span className="font-mono text-xs text-[var(--text-dim)]">{String(index + 1).padStart(2, '0')}</span><div><p className="font-sans text-sm leading-6 text-[var(--text-secondary)]">{claim.text}</p><p className="mt-1 font-mono text-[10px] uppercase tracking-[0.1em] text-[var(--text-dim)]">Queries {claim.sourceQuerySequences.join(', ') || '—'}</p></div></div>)}</div> : <p className="font-sans text-sm text-[var(--text-tertiary)]">No supported items were identified.</p>}
    </section>
  );
}
...
export default function InvestigationReportPage() {
  ...
  return <AppShell><main className="mx-auto w-full max-w-3xl px-4 py-8 md:px-8 md:py-12">
    ...
    {report && <article><div className="mb-8 border-b border-[var(--border-default)] pb-8"><div className="mb-3 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--text-dim)]"><FileText size={13} aria-hidden="true" /> Knowledge report</div><h1 className="font-sans text-3xl font-semibold tracking-tight text-[var(--text-primary)] md:text-4xl">{report.title}</h1><p className="mt-5 font-sans text-base leading-7 text-[var(--text-secondary)]">{report.executiveSummary}</p></div><ClaimList title="Key findings" claims={report.keyFindings} /><ClaimList title="Opportunities" claims={report.opportunities} /><ClaimList title="Risks" claims={report.risks} /><section className="border-t border-[var(--border-default)] py-6"><h2 className="mb-4 font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-[var(--text-dim)]">Open questions</h2>{report.openQuestions.length ? <ul className="space-y-2">{report.openQuestions.map((q) => <li key={q} className="font-sans text-sm leading-6 text-[var(--text-secondary)]">{q}</li>)}</ul> : <p className="font-sans text-sm text-[var(--text-tertiary)]">No open questions recorded.</p>}</section><section className="border-t border-[var(--border-default)] py-6"><h2 className="mb-4 font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-[var(--text-dim)]">Entities of interest</h2><div className="flex flex-wrap gap-2">{report.entitiesOfInterest.map((entity) => <span key={entity} className="rounded-full border border-[var(--border-default)] px-3 py-1.5 font-sans text-xs text-[var(--text-secondary)]">{entity}</span>)}</div></section></article>}
  </main></AppShell>;
}
